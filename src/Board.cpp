...
void Board::printBoard() const
{
    ...
    for (int r = 7; r >= 0; --r)
    {
        ...
        for (int c = 0; c < 8; ++c)
        {
            if (Piece* p = board[r][c]) std::cout << p->getChar();
            else                        std::cout << '.';
            std::cout << ' ';
        }
        std::cout << 8 - r << '\n';
...
Piece* Board::execute(const std::string& fromStr, const std::string& toStr) {
    Position from(fromStr), to(toStr);

    if (!Board::inBounds(from.row, from.col) || !Board::inBounds(to.row, to.col)) {
        throw InvalidMoveException("Position out of bounds");
    }

    Piece* piece = board[from.row][from.col];
    if (!piece) throw InvalidMoveException("No piece at source");

    auto legalMoves = piece->getLegalMoves(*this, from);
    bool valid = false;
    ...
    if (!valid) throw InvalidMoveException("Illegal move for this piece");
...
void Board::undo(const std::string& from, const std::string& to, Piece* captured)
{
    Position pFrom(from), pTo(to);
    board[pFrom.row][pFrom.col] = board[pTo.row][pTo.col];
    board[pTo.row][pTo.col]     = captured;
    whiteTurn = !whiteTurn;
}
